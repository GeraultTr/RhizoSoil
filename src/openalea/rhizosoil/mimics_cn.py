...
def get_parameters(annual_NPP_C, clay_pct, Tsoil, input_lignin_pct, input_CN):
    ...
    ANPP_C  = annual_NPP_C ### NOTE Tristan investigate on which process this impacts, I think litter inputs and it should probably be cut for short term simulations
    ...
    tsoi  = Tsoil ##LTERdata$MAT  ### NOTE Tristan variable input
    ...
    desorb   = 0.05*desorb
    Nleak   = 0.2#.1   #This is the proportion N lost from DIN pool at each hourly time step. ### NOTE Tristan probably to handle differently if just leaving the system
...
@njit
def get_parameters_rhizodeposits(annual_NPP_C, annual_RD_C, clay_pct, Tsoil, input_RD_CN, input_LIT_CN):
    """
    All parameter values and comments are from `siteSpecificParameters`
    in "MIMICS-CN_forRelease.Rmd" (Kyker-Snowman, 2019).
    Comments with double-hash ## are lines of code removed by asb219.
    ...
    fMET        = fMET1
    Inputs        = np.empty(2, np.float64)           #Litter inputs to MET/STR
    Inputs[0]     = annual_RD_C / (365*24)      #partitioned to layers
    Inputs[1]     = annual_NPP_C / (365*24)  ## units: mgC/cm3/h
    FI       = np.array((0.05,0.3))#c(0.05, 0.05)#
...
def FXEQ(state, Fm_input,
    ...
    N_leaching = True
    if N_leaching:
        LeachingLoss = Nleak*DIN ### NOTE Tristan 0. here since this will be handled by the transport model
...
@njit
def FXEQ_no_label(state,
        Inputs, VMAX, KM, CUE, fPHYS, fCHEM, fAVAI, FI, turnover, desorb,
        KO, NUE, CN_m, CN_s, CN_r, CN_K, Nleak, densDep, RootUptakeIN=0., time_step_in_hours=1):
    """
    Most of the code is copied and adapted from the `FXEQ` function
    in "MIMICS-CN_forRelease.Rmd" (Kyker-Snowman, 2019).
    Addition by asb219: 14C implementation (last 4 blocks of code).
...
class MIMICS_CN:
    ...
    def __init__(self, time_step_in_hours=1, clay_percentage=30.):

        self.time_step_in_hours = time_step_in_hours
        self.clay_percentage = clay_percentage
        prop = 0.5
        parameters = get_parameters_rhizodeposits(
            annual_NPP_C = (1-prop)*100., # gC/m2/y
            annual_RD_C = prop*100., # gC/m2/y
        ...
        # First guess of steady state
        steady_state_C_N_guess = initialize_pools_no_label()

        # Find steady-state C and N stocks
        def func(state_C_N):
            return FXEQ_no_label(state_C_N, *parameters, RootUptakeIN=0., time_step_in_hours=time_step_in_hours)
        steady_state_C_N, success_C_N = self._find_steady_state(
            func, steady_state_C_N_guess, bounds=(0, 1e7), name='C,N'
        )

        self.state = steady_state_C_N if success_C_N else steady_state_C_N_guess
    ...
    def __call__(self, soil_temperature=10, labile_OC=5e-1, labile_ON=1e-2, labile_IN=1e-6, net_N_uptake=0., litter_inputs=50., litter_CN=15, rhizodeposits_inputs=50., rhizodeposits_CN=15.):
        ...
        self.state = np.maximum(state + FXEQ_no_label(state, *parameters, RootUptakeIN=net_N_uptake, time_step_in_hours=self.time_step_in_hours), 0.)
